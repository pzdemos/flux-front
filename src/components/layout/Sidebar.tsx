    ...
    <>
      ...
      >
        <div className={`flex flex-col h-full ${isMobile && !sidebarOpen ? 'invisible pointer-events-none' : ''}`}>
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between px-4'} py-4 border-b border-zinc-800`}>
            <button
              onClick={collapsed ? toggleSidebar : undefined}
              className={`flex items-center ${collapsed ? '' : 'gap-3'} ${collapsed ? 'cursor-pointer' : 'cursor-default'}`}
              title={collapsed ? '展开侧边栏' : undefined}
            >
              <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center">
                <FolderOpen className="w-5 h-5 text-white" />
              </div>
              {!collapsed && <span className="text-lg font-bold text-white tracking-tight">Flux</span>}
            </button>
            {!isMobile && !collapsed && (
              <button
            ...
            {modules.map((mod) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center ${collapsed ? 'justify-center px-0' : 'gap-3 px-3'} py-2.5 rounded-lg text-sm font-medium transition-colors cursor-default
                    ${isActive
          ...
          <div className={`border-t border-zinc-800 py-3 space-y-1 ${collapsed ? 'px-2' : 'px-3'}`}>
            <div className={`flex items-center ${collapsed ? 'justify-center px-0' : 'gap-3 px-3'} py-2`} title={collapsed ? user?.username || 'admin' : undefined}>
            ...
            <button
              ...
              className={`w-full flex items-center ${collapsed ? 'justify-center px-0' : 'gap-3 px-3'} py-2 rounded-lg text-sm text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/60 transition-colors`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center ${collapsed ? 'justify-center px-0' : 'gap-3 px-3'} py-2 rounded-lg text-sm text-zinc-400 hover:text-red-400 hover:bg-red-500/10 transition-colors`}
            >
